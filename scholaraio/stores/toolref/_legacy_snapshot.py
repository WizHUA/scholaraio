...
用法::
    ...
    from scholaraio.stores.toolref import toolref_fetch, toolref_show, toolref_search
...
def _toolref_root(cfg: Config | None = None) -> Path:
    if cfg is not None:
        toolref_root = getattr(cfg, "toolref_root", None)
        if toolref_root is not None:
            return Path(toolref_root)
        return Path(getattr(cfg, "_root", Path.cwd())) / "data" / "toolref"
